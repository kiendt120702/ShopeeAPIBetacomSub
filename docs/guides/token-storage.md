...
When using the SDK in a browser environment (React, Vue, etc.), there are some important differences from Node.js:

### Available Storage Options

| Storage Type | Browser | Node.js | Persistence | Security |
|-------------|---------|---------|-------------|----------|
| LocalStorage | ✅ | ❌ | Persistent | Low |
| Encrypted LocalStorage | ✅ | ❌ | Persistent | Medium |
| IndexedDB | ✅ | ❌ | Persistent | Low |
| Memory | ✅ | ✅ | Session only | High |
| Supabase | ✅ | ✅ | Persistent | High |
| File System | ❌ | ✅ | Persistent | Medium |

### Recommended Setup for Browser Apps

```typescript
import { createSecureStorage, createAutoStorage } from '@/lib/shopee/storage';

// Option 1: Auto-select based on environment
const storage = createAutoStorage(shopId);

// Option 2: Use Supabase for authenticated users (recommended)
const storage = createSecureStorage({
  shopId: 789012,
  userId: 'user-uuid-from-auth',
});

// Option 3: Use encrypted localStorage
const storage = createTokenStorage('encrypted', {
  shopId: 789012,
  encryptionKey: import.meta.env.VITE_TOKEN_ENCRYPTION_KEY,
});
```

### Security Best Practices for Browser
...
2. **Use encrypted storage in production**
   ...
   Add to `.env`:
   ```
   VITE_TOKEN_ENCRYPTION_KEY=your_generated_key
   ```

3. **Prefer server-side token storage**
   - Use Supabase storage for multi-device sync
   - Tokens are protected by RLS policies

4. **Handle token expiration gracefully**
   ```typescript
   // Auto-refresh wrapper
   async function withValidToken<T>(fn: (token: AccessToken) => Promise<T>): Promise<T> {
...
If migrating from Node.js file-based storage to browser:

```typescript
// The SDK's default file storage (.token/ folder) is NOT available in browsers
// Use one of the browser-compatible storage options instead

// For React apps with Supabase auth:
import { SupabaseTokenStorage } from '@/lib/shopee/storage';

const storage = new SupabaseTokenStorage(shopId, userId);
const sdk = new ShopeeSDK(config, storage);
```
