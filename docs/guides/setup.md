...
By default, the SDK stores tokens in local JSON files in a `.token` folder (Node.js) or localStorage (Browser). You can provide your own token storage implementation:

### Available Storage Types

| Type | Environment | Description |
|------|-------------|-------------|
| `localStorage` | Browser | Default for browser apps |
| `encrypted` | Browser | Encrypted localStorage (recommended for production) |
| `indexedDB` | Browser | IndexedDB storage |
| `memory` | Both | In-memory storage (session only) |
| `supabase` | Both | Supabase database storage (recommended for multi-device) |
...
// Use Supabase for authenticated users (recommended)
...
```typescript
import { setStorageType, setStorageOptions } from '@/lib/shopee/config';

// Set storage type globally
setStorageType('supabase', { userId: 'user-uuid' });

// Or just update options
setStorageOptions({ userId: 'user-uuid' });
```
