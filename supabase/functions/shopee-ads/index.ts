  ...
  try {
    ...
    switch (action) {
      ...
      case 'get-hourly-performance': {
        ...
        // performance_date: string (required) - Format DD-MM-YYYY
        if (!params.performance_date) {
          return new Response(JSON.stringify({ error: 'performance_date is required (DD-MM-YYYY)' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
      ...
      case 'get-daily-performance': {
        ...
        if (!params.start_date || !params.end_date) {
          return new Response(JSON.stringify({ error: 'start_date and end_date are required (DD-MM-YYYY)' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
      ...
      case 'get-campaign-hourly-performance': {
        ...
        const hourlyIds = Array.isArray(params.campaign_id_list)
          ? params.campaign_id_list.join(',')
          : params.campaign_id_list;
