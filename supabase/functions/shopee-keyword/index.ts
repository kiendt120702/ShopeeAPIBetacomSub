 ...
 * 
 * Endpoints:
 * - get_recommended_keyword_list: Lấy danh sách từ khóa đề xuất với search volume
 */
...
async function callShopeeAPIWithRetry(
  ...
  const makeRequest = async (accessToken: string) => {
    ...
    console.log('Calling Shopee Keyword API:', path);
  ...
  try {
    ...
    switch (action) {
      case 'get-recommended-keyword-list': {
        // Lấy danh sách từ khóa đề xuất
        // item_id: number (required) - ID sản phẩm
        // input_keyword: string (optional) - Từ khóa người dùng nhập
        if (!params.item_id) {
